...
processed_files_log = os.path.join(output_directory, "processed_files.json")
...
# 限制最大并发数
MAX_WORKERS = 5  # 限制线程数，防止 API 服务器过载
...
def process_file(file_path):
    ...
    # 调用 API 处理
    modified_text = get_deepseek_response(text)
...
def batch_process():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        list(
            tqdm(
                executor.map(process_file, txt_files),  # 这里改为传入完整路径
                total=len(txt_files),
    ...
    # 删除 `processed_files.json`
    if os.path.exists(processed_files_log):
        os.remove(processed_files_log)
        print(f"🗑️ 已删除文件: {processed_files_log}")
...
if __name__ == "__main__":
    ...
    print(f"✅ 正在从目录读取文件: {input_directory}")
    print(f"📂 目录中的文件: {os.listdir(input_directory)}")
    batch_process()
